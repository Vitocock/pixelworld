export default function ProductCatalog({ Product, onOpen, color }) {
  ...
  return (
    <div
      style={{ borderColor }}
      className="flex flex-col w-1/5 p-4 h-[32rem] border-2 transition-colors duration-1000 
    ...
    >
      ...
      />
      <div className="mt-4">
        <h2 className="font-['exo'] text-md">{brand}</h2>
        <h1 className="font-['orbitron'] leading-none text-2xl line-clamp-2 break-words min-h-[3rem]">{name}</h1>
      </div>
      <div className="w-4/5 h-[0.0625rem] my-2 self-center bg-slate-700"></div>
      <div className="font-['exo'] text-2xl">${base_price}</div>
      
      <div className="w-full mt-4 flex fixed left-7 bottom-4">
        <button
          onClick={onOpen}
          style={{ backgroundColor: borderColor }}
          className="w-4/5 h-10 px-4 py-1 text-white text-sm rounded-md transition-all duration-1000 hover:brightness-70"
        >
          Ver más
        </button>
      </div>
