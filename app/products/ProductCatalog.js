export default function ProductCatalog({ Product, onOpen, color }) {
  ...
  return (
    <div
      style={{ borderColor }}
      className="flex flex-col w-[44%] lg:w-1/5 p-4 lg:h-[32rem] border-2 transition-colors duration-500 
    ...
    >
      ...
      />
      <div className="mt-4">
        <h2 className="font-['exo'] text-xs lg:text-base">{brand}</h2>
        <h1 className="font-['orbitron'] leading-none text-base lg:text-2xl line-clamp-2 break-words min-h-[1.5rem] sm:min-h-[3rem]">{name}</h1>
      </div>
      <div className="w-4/5 h-[0.0625rem] my-2 self-center bg-slate-700"></div>
      <div className="font-['exo'] text-base lg:text-2xl">${base_price}</div>
